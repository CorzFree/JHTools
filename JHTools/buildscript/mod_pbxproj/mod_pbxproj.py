...
def load_mobileprovision(provisionfile):
    with open(os.devnull, 'w') as devnull:
        plistcontent = subprocess.check_output("/usr/bin/openssl smime -in \"%s\" -inform der -verify"%provisionfile, stderr=devnull, shell=True)
...
def install_provision(provisionfile):
    provisionInfo = load_mobileprovision(provisionfile)
    uuid = provisionInfo.get('UUID')

    provisionDir = os.path.expanduser("~/Library/MobileDevice/Provisioning Profiles/")
    if not os.path.exists(provisionDir):
        os.makedirs(provisionDir)

    install_path = provisionDir + uuid + ".mobileprovision"

    if os.path.isfile(install_path):
        if os.stat(install_path).st_mtime < os.stat(provisionfile).st_mtime:
            os.remove(install_path)
            shutil.copy2(provisionfile, install_path)
    else:
        shutil.copy2(provisionfile, install_path)

    return load_mobileprovision(install_path)
...
class PBXNativeTarget(PBXTarget):
    ...
    def set_development_team(self, team, manual=False):
        target_attribute = self.get_attributes()

        target_attribute['DevelopmentTeam'] = team

        if manual:
            target_attribute['ProvisioningStyle'] = 'Manual'
        elif 'ProvisioningStyle' in target_attribute:
            target_attribute.pop('ProvisioningStyle')
    ...
    def add_capabilities(self, capabilities):
        ...
        system_capabilities = target_attribute.get("SystemCapabilities")

        if system_capabilities == None:
            system_capabilities = PBXDict()
            target_attribute["SystemCapabilities"] = system_capabilities

        for item in capabilities:
            system_capabilities[item] = PBXDict(dict(enabled = "1"))

    def set_buildconfig_value(self, key, value, configName=None):
        if configName == None:
            for buildConfig in self.get_buildconfigs():
                buildConfig.add_single_valued_flag(key, value)
        else:
            buildConfig = self.get_buildconfig(configName)
            buildConfig.add_single_valued_flag(key, value)

    def get_buildconfig_value(self, key, configName='Release'):
        buildConfig = self.get_buildconfig(configName)
        return buildConfig["buildSettings"].get(key)

    def set_product_name(self, product_name, configName=None):
        self["productName"] = product_name

        if configName == None:
            for buildConfig in self.get_buildconfigs():
                buildConfig.add_single_valued_flag('PRODUCT_NAME', product_name)
        else:
            buildConfig = self.get_buildconfig(configName)
            buildConfig.add_single_valued_flag('PRODUCT_NAME', product_name)

    def get_product_name(self, configName="Release"):
        buildConfig = self.get_buildconfig(configName)
        product_name = buildConfig["buildSettings"]["PRODUCT_NAME"]
...
class XCBuildConfiguration(PBXType):
    ...
    def add_forceload_libs(self, libs):
        modified = False
        base = 'buildSettings'
        key = 'OTHER_LDFLAGS'
        ...
        if key in self[base] and ('-ObjC' in self[base][key] or '-all-load' in self[base][key]):
            return  False

        for flag in libs:
            if not flag.startswith('-force_load'):
                flag = '-force_load ' + flag
