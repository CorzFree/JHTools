...
#define JHToolsMainThreadAssert() NSAssert([NSThread isMainThread], @"JHToolsProgressHUD needs to be accessed on the main thread.");

CGFloat const JHToolsProgressMaxOffset = 1000000.f;

static const CGFloat JHToolsDefaultPadding = 4.f;
static const CGFloat JHToolsDefaultLabelFontSize = 16.f;
static const CGFloat JHToolsDefaultDetailsLabelFontSize = 12.f;


@interface JHToolsProgressHUD () {
    // Deprecated
...
@interface JHToolsProgressHUDRoundedButton : UIButton
@end


@implementation JHToolsProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    JHToolsProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    JHToolsProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (JHToolsProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (JHToolsProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = JHToolsProgressHUDAnimationFade;
    _mode = JHToolsProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(JHToolsProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == JHToolsProgressHUDAnimationZoom) {
        type = animatingIn ? JHToolsProgressHUDAnimationZoomIn : JHToolsProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == JHToolsProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == JHToolsProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == JHToolsProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == JHToolsProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    JHToolsProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<JHToolsProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    JHToolsBackgroundView *backgroundView = [[JHToolsBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = JHToolsProgressHUDBackgroundStyleSolidColor;
    ...
    JHToolsBackgroundView *bezelView = [JHToolsBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:JHToolsDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:JHToolsDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;
    
    UIButton *button = [JHToolsProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:JHToolsDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[JHToolsRoundProgressView class]];
    
    JHToolsProgressHUDMode mode = self.mode;
    if (mode == JHToolsProgressHUDModeIndeterminate) {
    ...
    else if (mode == JHToolsProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[JHToolsBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JHToolsProgressHUDModeDeterminate || mode == JHToolsProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[JHToolsRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == JHToolsProgressHUDModeAnnularDeterminate) {
            [(JHToolsRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == JHToolsProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JHToolsProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[JHToolsProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[JHToolsProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[JHToolsRoundProgressView class]]) {
        JHToolsRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [JHToolsRoundProgressView appearanceWhenContainedIn:[JHToolsProgressHUD class], nil];
#else
        appearance = [JHToolsRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[JHToolsProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((JHToolsRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((JHToolsRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[JHToolsBarProgressView class]]) {
        JHToolsBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [JHToolsBarProgressView appearanceWhenContainedIn:[JHToolsProgressHUD class], nil];
#else
        appearance = [JHToolsBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[JHToolsProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((JHToolsBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((JHToolsBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    JHToolsBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? JHToolsDefaultPadding : 0.f;
...
- (void)setMode:(JHToolsProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == JHToolsProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation JHToolsRoundProgressView
...
@implementation JHToolsBarProgressView
...
@interface JHToolsBackgroundView ()
...
@implementation JHToolsBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = JHToolsProgressHUDBackgroundStyleBlur;
            ...
            _style = JHToolsProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(JHToolsProgressHUDBackgroundStyle)style {
    if (style == JHToolsProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = JHToolsProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    JHToolsProgressHUDBackgroundStyle style = self.style;
    if (style == JHToolsProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == JHToolsProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation JHToolsProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [JHToolsProgressHUD allHUDsForView:view];
    for (JHToolsProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable JHToolsProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    JHToolsBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == JHToolsProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    JHToolsMainThreadAssert();
    self.backgroundView.style = JHToolsProgressHUDBackgroundStyleSolidColor;
...
@implementation JHToolsProgressHUDRoundedButton
